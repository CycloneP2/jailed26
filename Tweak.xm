...
- (void)drawEntity:(uintptr_t)entity withCam:(void*)cam w2s:(Vector3(*)(void*, Vector3))w2s 
    ...
    if (showEnemyHp) {
        ...
        if (maxHp > 0) {
            ...
            NSString *hpText = [NSString stringWithFormat:@"Γ¥ñ∩╕Å %d", hp];
...
- (void)drawMonster:(uintptr_t)entity withCam:(void*)cam w2s:(Vector3(*)(void*, Vector3))w2s 
    ...
    int m_id = *(int*)(entity + OFF_ENTITY_ID);
    NSString *monsterName = @"≡ƒÉë MONSTER";
    if (m_id == 1001 || m_id == 1002) monsterName = @"≡ƒææ LORD";
    else if (m_id == 2001) monsterName = @"≡ƒÉó TURTLE";
    else if (m_id == 3001 || m_id == 3002) monsterName = @"ΓÜí BUFF";
...
- (void)createMenu {
    ...
    [closeBtn setTitle:@"Γ£ò" forState:UIControlStateNormal];
    ...
    addToggle(@"≡ƒÄ» ESP MASTER", &espEnabled, 60);
    addToggle(@"≡ƒôª Enemy Box", &showEnemyBox, 100);
    addToggle(@"Γ¥ñ∩╕Å Enemy HP", &showEnemyHp, 140);
    addToggle(@"≡ƒÅ╖∩╕Å Enemy Name", &showEnemyName, 180);
    addToggle(@"≡ƒôÅ Distance", &showDistance, 220);
    addToggle(@"≡ƒöù Snapline", &showEnemyLine, 260);
    addToggle(@"≡ƒæÑ Show Team", &showTeamEsp, 300);
    addToggle(@"≡ƒÉë Monster ESP", &showMonsterEsp, 340);
...
%ctor {
    %init;
    NSLog(@"Γ£à EDGY ESP MLBB LOADED - OFFSET UPDATED v2.1.67.1173.1");
}
